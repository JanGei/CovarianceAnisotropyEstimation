...
def plot_k_fields(gwf: flopy.mf6.modflow.mfgwf.ModflowGwf, pars,  k_fields: list):
    ...
    for i in range(int(len(k_fields)/2)):
        for j, letter in enumerate(['r', 'l']):
            ...
            c = axf.plot_array(np.log(gwf.npf.k.array), cmap=cm.bilbao_r, alpha=1, vmin = kmin, vmax = kmax)
            ax.set_aspect('equal')
    ...
    ax = axes['b']
    axf = flopy.plot.PlotMapView(model=gwf, ax=ax)
    c = axf.plot_array((gwf.npf.k.array), cmap=cm.bilbao_r, alpha=1, vmin = kmin, vmax = kmax)
    ax.set_aspect('equal')
    plt.tight_layout()
    plt.show()
