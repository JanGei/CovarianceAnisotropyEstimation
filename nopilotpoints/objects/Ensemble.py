    ...
    def apply_X(self, params: list, X):
        ...
        if 'cov_data' in params:
            ...
            if self.pilotp_flag:
                ...
                self.meank = np.mean(X[:self.members[0].npf.k.array.size,:], axis = 1)
        ...
        for i in range(self.n_mem):
            if 'cov_data' in params:
                ...
                if self.pilotp_flag:
                    ...
                    x = np.concatenate((data[i]['npf'].flatten(),
                                        head[i]['h']))
    ...
    def record_state(self, pars: dict, params: list):
        ...
        if 'npf' in params:
            if self.pilotp_flag:
                ...
                f = open(os.path.join(direc,  'k_mean.dat'),'a')
